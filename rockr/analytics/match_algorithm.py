...
from rockr.models import User, MatchProfile, UserInstrument, UserMusicalInterest, UserGoal
...
def match_algorithm(instrumentids, musicinterestids, goalids):
    ...
    match_users = User.query.filter_by(is_admin=False,is_active=True,is_band=False).all()
    ...
    for matched_user in match_users:
        ...
        if len(instrumentids) == 0:
            ...
            for instrumentid in instrumentids:
                match_instruments = UserInstrument.query.filter_by(id=match_user_id,instrument_id=instrumentid).first()
        ...
        if len(musicinterestids) == 0:
            ...
            for musicinterestid in musicinterestids:
                match_music_interest = UserMusicalInterest.query.filter_by(id=match_user_id,interest_id=musicinterestid).first()
        ...
        if len(goalids) == 0:
            ...
            for goalid in goalids:
                match_goal = UserGoal.query.filter_by(id=match_user_id,goal_id=goalid).first()
        ...
        match_tuple = (match_score,matched_user.id,matched_user.username,matched_user.first_name,matched_user.last_name)
        #append to the list
